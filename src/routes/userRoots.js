...
router.get('/products', async (req, res) => {
  try {
      const [results, fields] = await db.promisePool.execute('SELECT ProductID, Name, Description, Price, ImageURL FROM Product');
      res.json(results);
  } catch (err) {
...
router.post('/orders', (req, res) => {
    ...
    db.promisePool.execute(insertQuery, [ProductId, CustomerName, CustomerEmail, CustomerPhoneNumber, CustomerAddress], (err, results) => {
      if (err) {
